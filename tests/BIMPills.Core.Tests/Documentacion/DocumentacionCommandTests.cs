...
{
    public class DocumentacionCommandTests
    {
        [Fact]
        public void Execute_WithValidContext_ReturnsSuccess()
        {
            var context = new FakeDocContext(new FakeDocDocServices());
            var command = new DocumentacionCommand();
    ...
    internal sealed class FakeDocContext : ICommandContext
    {
        public IDocumentServices Document { get; }
        public ILogger Logger { get; } = new NullDocLogger();
        public FakeDocContext(IDocumentServices doc) { Document = doc; }
    }

    internal sealed class FakeDocDocServices : IDocumentServices
    {
    ...
    internal sealed class NullDocLogger : ILogger
    {
