...
namespace BIMPills.Core.Tests.ExportFamilies
{
    public class ExportFamiliesCommandTests
    {
        [Fact]
        public void Execute_WithValidDocument_ReturnsSuccess()
        {
            var context = new FakeContext(new FakeDocServices());
            var command = new ExportFamiliesCommand();
        ...
        [Fact]
        public void Execute_StoresLastResult()
        {
            var context = new FakeContext(new FakeDocServices());
            var command = new ExportFamiliesCommand();
            command.Execute(context);

            Assert.NotNull(ExportFamiliesCommand.LastResult);
            Assert.Equal("TestModel.rvt", ExportFamiliesCommand.LastResult.DocumentTitle);
        }
    ...
    internal sealed class FakeContext : ICommandContext
    {
        public IDocumentServices Document { get; }
        public ILogger Logger { get; } = new NullLogger();
        public FakeContext(IDocumentServices doc) { Document = doc; }
    }

    internal sealed class FakeDocServices : IDocumentServices
    {
