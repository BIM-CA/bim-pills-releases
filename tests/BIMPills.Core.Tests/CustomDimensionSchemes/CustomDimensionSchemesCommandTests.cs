...
namespace BIMPills.Core.Tests.CustomDimensionSchemes
{
    public class CustomDimensionSchemesCommandTests
    {
        [Fact]
        public void Execute_WithValidContext_ReturnsSuccess()
        {
            ...
            var context = new FakeSchemesContext(new FakeSchemesDocServices());
            var command = new CustomDimensionSchemesCommand(schemeService);
    ...
    internal sealed class FakeSchemesContext : ICommandContext
    {
        public IDocumentServices Document { get; }
        public ILogger Logger { get; } = new NullSchemesLogger();
        public FakeSchemesContext(IDocumentServices doc) { Document = doc; }
    }

    internal sealed class FakeSchemesDocServices : IDocumentServices
    {
    ...
    internal sealed class NullSchemesLogger : ILogger
    {
